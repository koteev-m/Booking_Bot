...
fun buildBookingFsm( //
    ...
    val logger = LoggerFactory.getLogger("FSM.${this.name}") //
        ...
        transition<PeopleEnteredEvent>("PeopleToSlot") { //
            action { //
                ...
                if (event.count < BotConstants.MIN_GUESTS_DEFAULT || event.count > maxGuests) { //
                    ...
                    val baseTimeEnd = selectedDate.atTime(23,59).atZone(zone).toInstant().plus(23*60*60 + 59*60, ChronoUnit.SECONDS) // Example: End of day, ensure it's correct

                    // Placeholder slots, this needs proper implementation
                    val slots = listOf( //
                    ...
                    ).filter { it.second.isBefore(baseTimeEnd.plus(1, ChronoUnit.DAYS).truncatedTo(ChronoUnit.DAYS)) } // Ensure slots are within operational hours, adjust logic as needed //
    ...
    addState(FsmStates.EnterGuestName) { //
        ...
        transition<BackPressedEvent>("BackFromNameToSlot") { //
            ...
            action { //
                data.guestName = null //
                val strings = StringProviderFactory.get(data.currentLanguageCode) //
                // Placeholder, regenerate or fetch proper slots
                val selectedDate = data.selectedDate!! //
                val zone = ZoneId.of(data.club!!.timezone) //
                val baseTimeStart = selectedDate.atTime(22,0).atZone(zone).toInstant() //
                val baseTimeEnd = selectedDate.atTime(23,59).atZone(zone).toInstant().plus(23*60*60 + 59*60, ChronoUnit.SECONDS)

                val slots = listOf( //
                    baseTimeStart to baseTimeStart.plus(2, ChronoUnit.HOURS), //
                    baseTimeStart.plus(3, ChronoUnit.HOURS) to baseTimeStart.plus(5, ChronoUnit.HOURS) //
                ).filter { it.second.isBefore(baseTimeEnd.plus(1, ChronoUnit.DAYS).truncatedTo(ChronoUnit.DAYS)) } //
                scope.launch { deps.bot.sendChooseSlotKeyboard(event.chatId, data.club!!, data.tableInfo!!, slots, strings, data.currentMessageId, 5 to totalBookingSteps) } //
            }
        }
    }
    ...
    addState(FsmStates.ConfirmBooking) { //
        transition<ConfirmBookingPressedEvent>("ConfirmToFinished") { //
            ...
            action { //
                ...
                if (bookingEntity == null) { //
                    ...
                    val bookingDate = bookingEntity.dateStart.atZone(ZoneId.of(data.club!!.timezone)).toLocalDate() //
                    ...
                    if (!isFree) { //
                        targetState = FsmStates.ChooseTable // Go back to table selection if not free
                        scope.launch { //
                            deps.bot.sendErrorMessage( //
                                event.chatId, //
                                strings, //
                                "Этот стол уже забронирован на выбранную дату и время. Пожалуйста, выберите другой стол или время.", //
                                deps.clubsRepo.getAllClubs(activeOnly = true), //
                                data.currentMessageId //
                            )
                            // Resend table selection
                            val tables = deps.tablesRepo.listAvailableByClubAndDate(data.clubId!!, data.selectedDate!!, activeOnly = true) //
                            deps.bot.sendChooseTableKeyboard(event.chatId, tables, data.club!!, data.selectedDate!!, strings, data.currentMessageId, 3 to totalBookingSteps) //
                        }
                    } else { //
        ...
        transition<BackPressedEvent>("BackFromConfirmToPhone") { //
            guard = {event.targetStateName == FsmStates.EnterGuestPhone.name} //
            targetState = FsmStates.EnterGuestPhone //
            action { //
                val strings = StringProviderFactory.get(data.currentLanguageCode) //
                scope.launch { deps.bot.askGuestPhone(event.chatId, strings, data.currentMessageId, 7 to totalBookingSteps) } //
    ...
    addState(FsmStates.AskFeedback) { //
        ...
        transition<RateBookingEvent>("FeedbackRatingSubmitted") { //
            ...
            action { //
                val strings = StringProviderFactory.get(data.currentLanguageCode) //
                deps.bookingsRepo.addFeedback(event.bookingId, data.dbUser!!.id, event.rating!!, null) //
